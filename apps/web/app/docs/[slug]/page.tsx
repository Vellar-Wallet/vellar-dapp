...
export async function generateMetadata({
  ...
  if (!page) return { title: "Docs — VELA" };
  return { title: `${page.title} — VELA Docs` };
}
...
export default async function DocPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="docs-article">
      <p className="docs-eyebrow mono">Documentation</p>
      ...
      <nav className="docs-pager">
        {prev ? (
          <Link href={`/docs/${prev.slug}`} className="docs-pager-link neo">
        ...
        {next ? (
          <Link href={`/docs/${next.slug}`} className="docs-pager-link neo docs-pager-next">
