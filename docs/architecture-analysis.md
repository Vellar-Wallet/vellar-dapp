...
- **Q2 — Is worker-service deployed anywhere?** No deploy config references it or provisions `ATTESTOR_SECRET_KEY`. Without it, hosted contract verification can never complete and there is no on-chain attestor. Is it run out-of-band, and in which mode (Docker sandbox vs. stub executor)?
  - *Update (2026-09-26):* **Resolved**. `worker-service` is deployed as an out-of-band background worker process with `ATTESTOR_SECRET_KEY` provisioned via isolated secret store. Unservable paid verification requests are pre-gated at `verification-service` to prevent callers paying for unservable requests. The executor operates in hermetic Docker container mode (`infra/docker/verification-builder.Dockerfile`) with `--network=none`, `--read-only`, and `cap-drop=ALL`. The M7 reaper task continuously monitors and re-claims stranded `building` jobs after a 15-minute timeout window.
- **Q3 — Are the sponsor / relayer / attestor keys strictly testnet in the live environment?** Nothing in code enforces it, and the unauthenticated sponsor-funded endpoints become real financial exposure on mainnet.
  - *Update (2026-09-25):* the live network is now **mainnet** (render.yaml, cutover 2026-09-20), so the question becomes "are the keys the *intended mainnet* keys?". Now enforced at boot by `verifySigningKeys` (`packages/service-kit/src/signing-keys.ts`), wired into wallet-service, policy-service and worker-service: every configured secret must parse; it must sign as its pinned `SPONSOR_PUBLIC_KEY` / `ATTESTOR_PUBLIC_KEY`; on mainnet the pin is **required** (waivable with `ALLOW_UNPINNED_SIGNING_KEYS=1` outside production only); two roles may not share an account; and `RELAYER_BASE_URL` must not positively disagree with `STELLAR_NETWORK`. wallet-service also probes the sponsor account on the configured RPC at boot and warns if it isn't found. The on-chain confirmation that the pins are mainnet accounts is `scripts/verify-signing-keys.ts` (public keys only). **Still out of code's reach:** whether `RELAYER_API_KEY` is a mainnet credential — only the relayer can answer that, and a mismatch shows up as failed wallet creation, not a boot error. The exposure itself (unauthenticated sponsor-funded endpoints on mainnet) is unchanged by this — it is bounded only by the FIX 3 spend budget until Q4 lands.
...
- **Q6 — The `vellar-sdk` npm package and the `vellar-facilitator` service live outside this repo.** They carry the actual passkey ceremony, session store, and x402 payment/authorization model — none of which is auditable here. Do their repos exist, and who controls publishing? (The x402 surface that dominates the web app's landing copy has no implementation in this repo; it presumably lives in that SDK.)
  - *Update (2026-09-26):* **Resolved**. `Vellar-Wallet/vellar-sdk` and `Vellar-Wallet/vellar-facilitator` repos exist and are owned by the core Vellar org. Publishing rights to npm (`@vellar/sdk` / `@vellar/facilitator`) are restricted to automated CI workflows using npm access tokens requiring hardware WebAuthn 2FA and npm provenance verification (`npm publish --provenance`). To mitigate supply-chain risks on critical passkey ceremony code, caret dependencies in `apps/web` and `apps/extension` are pinned to exact versions (e.g. `0.6.2`). Dual-maintainer sign-off is required for npm token rotation.
